...
- **B**：开源了一个求职 Agent 工具：33 个顶层命令 + 7 个招聘者子命令 + 43 个 MCP tool，让 AI 替你找工作
...
#### 3.3 MCP Server：43 个工具拉通 Claude Desktop

MCP（Model Context Protocol）是 Anthropic 推的标准，让任意 CLI/服务变成 Claude 可调用的工具。`mcp-server/server.py` 把当前 CLI 暴露成 43 个 MCP tool，除了求职链路，还覆盖招聘者侧操作和 AI 助手能力，stdio 协议直通 Claude Desktop。
